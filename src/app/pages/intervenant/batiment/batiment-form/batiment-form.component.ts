...
@Component({
  ...
  styleUrls: ['./batiment-form.component.scss']
})
export class BatimentFormComponent {
  @Input() target?: BatimentResponseDTO;
  ...
  constructor(private fb: FormBuilder, private service: BatimentService, private toastr: ToastrService) {
  ...
  ngOnChanges(): void {
    if (this.mode === 'edit' && this.target) {
      this.form.patchValue({
        nom: this.target.nom,
        localisation: this.target.localisation
      });
    }
  }
  ...
  handleSubmit(): void {
    ...
    const dto: BatimentDTO = this.form.value;
    ...
    if (this.mode === 'create') {
      ...
      this.service.update(this.target.id, dto).subscribe({
        next: () => {
          this.toastr.success("Bâtiment mis à jour");
          this.onUpdate.emit();
          this.showForm = false;
          this.processing = false;
        },
